...
const questions_array = [
  {
    "question": "Which part of the world are there the most penguins?",
    "choices": ["Groenland", "Canada", "Antarctica"],
    "answer": "Antarctica"
    },
  {
    "question": "Where Christmas was born?",
    "choices": ["Italy", "England", "Austria"],
    "answer": "Italy"
    },
  {
    "question": "What do penguins eat most?",
    "choices": ["Krill", "Fish", "Otter"],
    "answer": "Krill"
    },
  {
  ...
  {
    "question": "Where were the first Winter Olympics held?",
    "choices": ["Austria", "France", "Canada"],
    "answer": "France"
    },
  {
    "question": "What country has a town that gets no sunlight during the Polar Night?",
    "choices": ["Sweden", "Iceland", "Norway"],
    "answer": "Norway"
    },
  {
    "question": "What months are considered winter in the southern hemisphere?",
    "choices": ["January to April", "May to August", "June to September"],
    "answer": "June to September"
    },
  {
    "question": "What temperature is on average in Brazil during Christmas?",
    "choices": ["5° to 15°", "15° to 25°", "25° to 35°"],
    "answer": "25° to 35°"
    },
  {
    "question": "What of these animals is not  specifically winter acclimated?",
    "choices": ["Red fox", "Polar bear", "Deer"],
    "answer": "Deer"
    },
  {
    "question": "What material is most often used to grit / thaw roads?",
    "choices": ["Sulfur dioxide", "Coal", "Salt"],
    "answer": "Salt"
   },
  {
    "question": "Who wrote the play ‘A Winter’s Tale’?",
    "choices": ["Ernest Hemingway", "William Shakespeare", "Oscar Wilde"],
    "answer": "William Shakespeare"
    },
  {
    "question": "What Country is the coldest in winter?",
    "choices": ["Canada", "Norway", "Russia"],
    "answer": "Canada"
    },
  {
    "question": "What country has received the most snowfall in the 21st century?",
    "choices": ["Russia", "Sweden", "Japan"],
    "answer": "Japan"
  },
]

var question_picked: Dictionary
@onready var button1 = $SubViewport/PopupPanel/VBoxContainer/Button1
@onready var button2 = $SubViewport/PopupPanel/VBoxContainer/Button2
@onready var button3 = $SubViewport/PopupPanel/VBoxContainer/Button3
@onready var question = $SubViewport/PopupPanel/VBoxContainer/Question

func _ready():
  randomize()
  
  question_picked = questions_array[randi() % questions_array.size()]
  button1.quizz_minigame = self; button2.quizz_minigame = self; button3.quizz_minigame = self
  question.text = question_picked["question"]
  for indx in range(1, question_picked["choices"].size() + 1):
    get_node("SubViewport/PopupPanel/VBoxContainer/Button%s" % indx).text = question_picked["choices"][indx - 1]
  
#@onready var popup = $SubViewport/PopupPanel
#@onready var gui_area = $GUI/Area3D
#@onready var subviewport_button = $SubViewport/PopupPanel/VBoxContainer/Button
#
## Called when the node enters the scene tree for the first time.
#func _ready():
  #popup.show()
  #print($Camera3D.project_position(Vector2(1000, 1), 30))
  #gui_area.connect("input_event", _on_GUI_area_input_event)
#
...
#func _process(delta):
    ...
    #popup.show()
#
#func _on_GUI_area_input_event(event) -> void:
  #if event is InputEventMouseButton:
    #if event.mouse_index == MOUSE_BUTTON_LEFT:
      #if event.pressed:
        #subviewport_button.emit_signal("button_down")
...
#@onready var node_quad = $GUI/MeshInstance3D
#@onready var node_area = $GUI/Area3D
#
#
#func _ready():
  #node_area.connect("mouse_entered", _mouse_entered_area)
#
  ## If the material is NOT set to use billboard settings, then avoid running billboard specific code
  #if node_quad.get_surface_override_material(0).billboard_mode == 0:
    #set_process(false)
#
#
#
...
#
  ...
  #var parameters = PhysicsRayQueryParameters3D.create(from, to, node_area.collision_layer, [])
  #parameters.collide_with_areas = true
  #parameters.collide_with_bodies = false
  #var result = get_world_3d().direct_space_state.intersect_ray(parameters) #for 3.1 changes
#
  #if result.size() > 0:
